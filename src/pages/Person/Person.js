...
const Person = () => {
    ...
    useEffect(() => {
        const fetchData = async() => {
            const responsePerson = await axios.get(`https://api.themoviedb.org/3/person/${params.id}?api_key=${process.env.REACT_APP_API_KEY}&language=en-US`);
    ...
    useEffect(() => {
        const fetchData = async () => {
            const responseMovies = await axios.get(`https://api.themoviedb.org/3/person/${params.id}/movie_credits?api_key=${process.env.REACT_APP_API_KEY}&language=en-US`);
    ...
    useEffect(() => {
        const fetchData = async() => {
            const responseTv = await axios.get(`https://api.themoviedb.org/3/person/${params.id}/tv_credits?api_key=${process.env.REACT_APP_API_KEY}&language=en-US`);
        ...
        <div className='container text-light'>
            <div className={`row ${styles['row-margin-top']}`}>
                <div className={`col-12 col-md-4 col-lg-3 ${styles['center-img']}`}>
                    {
                        person.profile_path &&  <img className={`${styles['img-radius']} ${styles['img-size']}`} src={`https://image.tmdb.org/t/p/original/${person.profile_path}`} alt='movie_img' />
                    }
                </div>

                <div className={`col-12 col-md-8 col-lg-9 ${styles['content-margin-top']}`}>
                    <div>
                        <a 
                    ...
                    <div className='row'>
                        <div className='mt-4 d-flex justify-content-between align-items-center'>
                            <h4>Movies</h4>
                            {
                                ...
                                <p onClick={showMoreMovieItemHandler} className={styles['show-btn']}>
                                    {
                                        !isShowMoreMovie ? 'Show More' : 'Show Less'
                                    }
                                </p>
                            }
                        </div>
                        {
                            movies.map((movie) => (
                                movie.poster_path && 
                                <Link key={movie.id} to={`/movie/${movie.id}`} className='col-6 col-sm-4 col-md-3 col-lg-2 text-decoration-none text-light'>
                        ...
                        <div className='row'>
                            <div className='mt-4 d-flex justify-content-between align-items-center'>
                                <h4>Tv Shows</h4>
                                {
                                    ...
                                    <p onClick={showMoreTvItemHandler} className={styles['show-btn']}>
                                        {
                                            !isShowMoreTv ? 'Show More' : 'Show Less'
                                        }
                                    </p>
                                }
                            </div>
                            {
                                tvShows.map((item) => (
                                    item.poster_path && 
                                    <div key={item.id} className='card bg-transparent col-6 col-sm-4 col-md-3 col-lg-2'>
                                        <img className='card-img-top rounded' src={`https://www.themoviedb.org/t/p/w220_and_h330_face/${item.poster_path}`} alt='movie-img'/>
                                        <p>{item.name}</p>
                                    </div>
