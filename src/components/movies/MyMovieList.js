                        ...
                        movieList.map((movie) => (
                            <div className="row mt-5 mb-3 shadow p-3 bg-body rounded" key={movie.id}>
                                ...
                                <div className="col-sm-12 col-md-6">
                                    ...
                                    <p>Release Date: <span className="text-danger">{movie.date}</span></p>
                                    <p className="fst-italic">You added this movie to your list <span className="text-danger">{movie.addDate}</span></p>
                                    
                                    <button style={{display: 'none'}} className={`${styles['show-button']} btn btn-danger`} onClick={() => deleteFromListHandler(movie.id)} >Delete
                                    </button> 
                                    {/* only show xs and sm */}
                                </div>
