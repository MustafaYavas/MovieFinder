	...
	return async (dispatch) => {
		const fetchData = async () => {
			const response = await fetch(
				'https://movies-2ce3f-default-rtdb.firebaseio.com/movielist/movie.json'
			);
...
export const sendCartData = (data) => {
	return async () => {
		const sendRequest = async () => {
			const response = await fetch(
				'https://movies-2ce3f-default-rtdb.firebaseio.com/movielist.json',
				{
					method: 'PUT',
					body: JSON.stringify({
						movie: data,
			...
			if (!response.ok) throw new Error('Sending Movie Datas Failed!');
		};
